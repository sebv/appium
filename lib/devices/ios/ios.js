...
IOS.prototype.navToInitialWebview = function (cb) {
  if (this.args.udid) {
    ...
    this.navToLatestAvailableWebview(function(err) {
      cb(err);
      //if(err) return cb(err);
      //var welcomeUrl = 'http://localhost:' + this.args.port + '/welcome';
      //var script = 'window.location.href="' + welcomeUrl + '";';
      //console.log('AKAK welcomeUrl ->', script);
      //this.execute(script, [], cb);
    }.bind(this));
  }
};
